<template>
  <ViewTable
    v-if="this.fovComputed.length > 0"
    :data="{
      columns: Object.keys(this.fovComputed[0]),
      values: this.fovComputed.map((e) => Object.values(e)),
    }"
  />
</template>
...
export default {
  ...
  computed: {
    ...
    fovComputed() {
      let entries = Object.entries(this.fovDataByObject);
